function [target_DIH] = computeTargetDIH_b2(ages,genders,logDIH,...
...
M = full([agesex_test,drugs_test,lab_test,cond_test,proc_test,...
    ...
    ndrugs_test,nlab_test,nprov_test,nvend_test,...
    npcp_test,extralos_test,nclaims_test,nspec_test,nplace_test,...
    nproc_test,ncond_test,extradsfs_test,exchar_test,extraprob_test]);
m_test = size(M,1);

disp('training ensemble');
try
  load(sprintf('cache/ens_b2_TREES%d_m%d.mat',NTREES,m));
catch
  r=randperm(m);
  logDIH = logDIH(r);
  A = A(r,:);
  ...
  for i=1:NFOLDS
    ...
    try
      load(sprintf('cache/ens_b2_TREES%d_m%d_iter%d.mat',NTREES,m,i));
    catch
      ...
      save(sprintf('cache/ens_b2_TREES%d_m%d_iter%d.mat',NTREES,m,i),...
  ...
  %save(sprintf('cache/ens_b2_TREES%d_m%d.mat',NTREES,m),'ens');
  save(sprintf('cache/ens_b2_TREES%d_m%d.mat',NTREES,m),'train_preds', ...
    'test_preds','err');
end
fprintf('computeTargetDIH_b2 %d Trees. TRAINING ERROR: %f\n',NTREES,err);
