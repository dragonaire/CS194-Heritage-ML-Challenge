function [target_DIH c vars] = computeTargetDIH_extradsfscharlson(ages,genders,logDIH,...
...
if ~strcmp(cvx_status,'Solved') && ~strcmp(cvx_status,'Inaccurate/Solved')
    'computeTargetDIH_extradsfscharlson failed'
    keyboard
end
err = sqrt(mean((logDIH-postProcess(A_pca*c)).^2));
fprintf('computeTargetDIH_extradsfscharlson TRAINING ERROR: %f\n',err);

c = hillClimb3(A_pca,c,logDIH);
err = sqrt(mean((logDIH-postProcess(A_pca*c)).^2));
fprintf('computeTargetDIH_extradsfscharlson HILLCLIMB TRAINING ERROR: %f\n',err);
target_DIH = M_pca*c;
