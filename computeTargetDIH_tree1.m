function [target_DIH] = computeTargetDIH_tree1(ages,genders,logDIH,...
...
NCOLS = 182
try
  load(sprintf('cache/tree1_m%d_ncols%d.mat', m,NCOLS));
catch
  rtree = RegressionTree.fit(A(:,1:NCOLS),logDIH);
  save(sprintf('cache/tree1_m%d_ncols%d.mat', m,NCOLS), 'rtree');
end
target_DIH = predict(rtree, M(:,1:NCOLS));
