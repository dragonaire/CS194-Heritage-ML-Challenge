function [target_DIH c vars] = computeTargetDIH_svm1(ages,genders,logDIH,...
...
cvx_begin quiet
    variables hosp not;
    minimize(norm([logDIHhosp-hosp; logDIHnot-not]))
    subject to
cvx_end
...
p_test = svmclassify(s,M);
target_DIH = hosp*ones(m_test,1); target_DIH(p_test==0) = not;
