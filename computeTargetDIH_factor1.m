  ...
  for BASE=BASES
    ...
    for n=ns
      ...
      for seed=seeds
            ...
            try
                ...
                try
                    ...
                    cvx_begin quiet
                        ...
                        subject to
                            x == reshape((A(:,nstart:n)*g)',DIM*m,1).*reshape((A(:,nstart:n)*f)',DIM*m,1);
                    cvx_end
        ...
        if HILLCLIMB
          x = reshape((A(:,nstart:n)*cur_g)',DIM*m,1).*reshape((A(:,nstart:n)*cur_f)',DIM*m,1);
          err = sqrt((norm(postProcess(B*x)-remLogDIH)^2)/m);
          disp(sprintf('factor1 preHILLCLIMB TRAINING ERROR: %f',err));
          [cur_f,cur_g] = hillClimbCatVec(A(:,nstart:n),B,cur_f,cur_g,remLogDIH,DIM);
          x = reshape((A(:,nstart:n)*cur_g)',DIM*m,1).*reshape((A(:,nstart:n)*cur_f)',DIM*m,1);
          err = sqrt((norm(postProcess(B*x)-remLogDIH)^2)/m);
          fprintf('factor1 HILLCLIMB TRAINING ERROR: %f\n',err);
        end

        x = reshape((M(:,nstart:n)*cur_g)',DIM*m_test,1).*reshape((M(:,nstart:n)*cur_f)',DIM*m_test,1);
