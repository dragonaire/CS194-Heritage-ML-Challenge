function [target_DIH c vars] = computeTargetDIH_condspeccombo(ages,genders,logDIH,...
    ages_test,genders_test,condspec_train,condspec_test)
constants;
...
[condspec_train,condspec_test] = condspecMap(condspec_train, condspec_test);
offsets = [...
    SIZE.AGE*SIZE.SEX,...
    size(condspec_train,2),...
    ];
...
A = [sparse(rows_i, cols_i, val, m, offsets(2)), condspec_train];
A_pca = sparse(A);
m_test = length(ages_test);
agesex_test = ages_test + 10*(genders_test-1);
M = [sparse(1:m_test,agesex_test,1,m_test,offsets(2)), condspec_test];
M_pca = sparse(M);
if PCA
    A = full(A);
    if ZSCORE
...
if ~strcmp(cvx_status,'Solved') && ~strcmp(cvx_status,'Inaccurate/Solved')
    'computeTargetDIH_condspeccombo failed'
    keyboard
end
err = sqrt(mean((logDIH-postProcess(A_pca*c)).^2));
fprintf('computeTargetDIH_condspeccombo TRAINING ERROR: %f\n',err);

c = hillClimb3(A_pca,c,logDIH);
err = sqrt(mean((logDIH-postProcess(A_pca*c)).^2));
fprintf('computeTargetDIH_condspeccombo HILLCLIMB TRAINING ERROR: %f\n',err);
target_DIH = M_pca*c;
target_DIH = exp(target_DIH)-1;
end
%TODO these functions make the arrays unsparse. Subtract the min value to
%make them sparse again
function [train_,test_] = condspecMap(train,test)
THRESH = 500;
