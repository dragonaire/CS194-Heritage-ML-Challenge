function [target_DIH] = computeTargetDIH_mars1(ages,genders,logDIH,...
...
try
  m = length(ages);
  load(sprintf('cache/computeTargetDIH_mars1_m%d_cols%d_groups%d.mat', m,NCOLS,NGROUPS));
catch
  constants;
  ...
  for j=1:length(offsets)-1
    for i=1:NGROUPS
      ...
      [model, time] = aresbuild(B, logB);
      p(:,j) = p(:,j) + arespredict(model, A(:,offsets(j)+1:offsets(j+1)));
      p_test(:,j) = p_test(:,j) + arespredict(model, M);
  ...
  if ~strcmp(cvx_status,'Solved') && ~strcmp(cvx_status,'Inaccurate/Solved')
      'computeTargetDIH_mars1 failed'
      keyboard
  end
  fprintf('b: %f, c: %f\n', b, c);
  optval = norm(logDIH-b-c*p);
  %fprintf('computeTargetDIH_mars1 TRAINING ERROR: %f\n',sqrt((cvx_optval^2)/m));
  fprintf('computeTargetDIH_mars1 TRAINING ERROR: %f\n',sqrt((optval^2)/m));

  target_DIH = b+c*p_test;
  target_DIH = exp(target_DIH)-1;
  save(sprintf('cache/computeTargetDIH_mars1_m%d_cols%d_groups%d.mat',m,NCOLS,NGROUPS), 'target_DIH');
end
end
